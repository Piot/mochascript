...
static size_t parse_array(mocha_parser* self, mocha_char end_char, mocha_error* error, const mocha_object* array[],
						  size_t array_max_count)
{
	size_t count = 0;
...
static const mocha_object* parse_map(mocha_parser* self, mocha_error* error)
{
	...
	size_t count = parse_array(self, '}', error, args, 128);
...
static const mocha_object* parse_number(mocha_parser* self, mocha_error* error)
{
	...
	if (is_floating_point) {
		...
		o = mocha_values_create_integer(self->values, (int) atol(s));
	}
	return o;
}
